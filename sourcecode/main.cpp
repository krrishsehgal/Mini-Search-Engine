...
vector<string> tokenize(const string &text, const unordered_set<string> &stopwords)
{
    ...
    while (ss >> word)
    {
        ...
        // Note: For autocomplete, we might want to index even stopwords,
        // but for search logic we keep skipping them.
...
{
    if (results.size() >= 5)
        return; // Limit suggestions to top 5
    if (node->isEndOfWord)
    {
                           ...
                           const unordered_set<string> &stopwords)
{
    ...
    {
        word_counts[token]++;
        insert_trie(root, token); // Add to Autocomplete Trie
    }
    ...
    const unordered_set<string> &stopwords)
{
    ...
    {
        ranked_results.push_back(entry);
    }
...
long long estimate_memory(const unordered_map<string, vector<DocInfo>> &index)
{
    ...
    {
        size += entry.first.capacity();                    // String size
        size += sizeof(vector<DocInfo>);                   // Vector overhead
        size += entry.second.capacity() * sizeof(DocInfo); // Data size
    }
    return size;
}
